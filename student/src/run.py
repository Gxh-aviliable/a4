...
def main():
    ...
    if args.function == 'pretrain':
        ...
        fine_dataset=dataset.NameDataset(pretrain_dataset,text_finetune)
        if args.reading_params_path is None :
            ...
            tconf=trainer.TrainerConfig(max_epochs=10,batch_size=256,learning_rate=args.finetune_lr,lr_decay=True,
                                        warmup_tokens=512*20,final_tokens=200*len(pretrain_dataset)*block_size,
                                        num_workers=4,writer=writer,ckpt_path=args.writing_params_path)
        finetuner = trainer.Trainer(model, fine_dataset, None, tconf)
        finetuner.train()
